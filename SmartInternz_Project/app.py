...
custom_theme = {
    "primaryColor": "#f63366",  # Customize the primary color
    "backgroundColor": "#f0f0f0",  # Customize the background color
    "secondaryBackgroundColor": "#f8f8f8",  # Customize the secondary background color
    "textColor": "#262730",  # Customize the text color
...
st.set_page_config(page_title="Web App", page_icon=":smiley:", layout="wide" )


# for dirname, _, filenames in os.walk('/laptop dataset'):
#     for filename in filenames:
#         print(os.path.join(dirname, filename))
dataframe = pd.read_csv(r'laptop.csv')
def main():
    activities = ['Select', 'Data', 'Prediction', 'About Project' , 'About Us']
    choices = st.sidebar.selectbox("Select choices" ,activities)
    df = pickle.load(open(r'processed_data.pickle','rb'))
    pipe = pickle.load(open(r'ml_model_pipe.pickle', 'rb'))
    if choices == 'Select':
        st.title('Welcome to Laptop Price Prediction Web Application')
        st.header('Data -')
        st.info('Shows Laptop data with Price and other specifications')
        ...
        st.header('About Us -')
        st.info('A brief Introduction about our team')
        ...
        st.write('''
                    1. Narendra 
                    - Reg. No : 20BEC0726
                    - Email : narenda.2020@vitstudent.ac.in
                    2. Akash Singh
                    - Reg. No : 20BEE0050
                    - Email : akash.singh2020@vitstudent.ac.in
                    3. Maneel Chauhan
                    - Reg. No : 20BEE0039
                    - Email : maneel.chauhan2020@vitstudent.ac.in
                    4. Ritik Kumar
                    - Reg. No : 20BEE0019
                    - Email : ritik.kumar2020@vitstudent.ac.in
                    ''')
